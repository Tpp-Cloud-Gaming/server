...
export class SubscriberController {
  ...
  async broadcastMessage(message) {
    ...
    for (let [subscriber, ws] of Object.entries(
      this.subscribers.getAllSubscribers(),
    )) {
  ...
  async sendEndSessionNotification(
    sessionTime,
    connectedClients,
    connectedOfferers,
    offerer,
    ...
    if (connectedOfferers[offerer]) {
      ...
      connectedOfferers[offerer].send(message);
    } else {
      console.log(`Offerer ${offerer} not found`);
    }

    if (connectedClients[client]) {
      connectedClients[client].send(message);
    } else {
  ...
  async removeSubscriber(ws) {
    for (let [subscriber, value] of Object.entries(
      this.subscribers.getAllSubscribers(),
    )) {
  ...
  async sendForceStopSessionNotification(
    sessionTerminator,
    offerer,
    client,
    sessionTime,
